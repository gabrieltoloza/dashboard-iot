...
<template>
    <svg :width="width" :height="height" :viewBox="`0 0 ${width} ${height}`" class="overflow-visible">
        ...
        <template v-if="type === 'line'">
            ...
            <path :d="points" :stroke="color" stroke-width="1.5" fill="none" stroke-linecap="round"
                stroke-linejoin="round" />
        </template>
        <template v-else>
            <rect v-for="(bar, i) in bars" :key="i" :x="bar.x" :y="bar.y" :width="bar.w" :height="bar.h" :fill="color"
                rx="1" />
        </template>
    </svg>
</template>