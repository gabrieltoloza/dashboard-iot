...
import { ArrowLeft, RefreshCw, Settings, Download, Bell } from 'lucide-vue-next'
...
onMounted(() => {
  devicesStore.fetchDevice(deviceId.value)
...
<template>
  <div class="space-y-6">
        ...
        <UiCardContent>
          ...
          <div v-else class="space-y-2">
            ...
            >
              ...
              <span class="text-xs text-muted-foreground whitespace-nowrap">{{ formatTime(alarm.fecha) }}</span>
            </div>
          </div>
        </UiCardContent>
      </UiCard>
    ...
    <div v-else class="py-12">
      <AppEmptyState title="Dispositivo no encontrado" description="Verifica que el ID sea correcto" icon="error" />
    </div>
  </div>
</template>
