...
const latestPolling = usePolling(() => sensorsStore.fetchLatest(), 30000)
...
onMounted(async () => {
    ...
    try {
        ...
        if (typeof console !== 'undefined') console.warn('[dashboard] loadSparklines failed', e)
    }
    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <UiCard v-for="device in devicesStore.devices" :key="device.id"
                    ...
                    <UiCardContent class="space-y-4">
                        ...
                        <div v-if="deviceSensorVisuals[device.id]?.length" class="space-y-3">
                            <div v-for="vis in deviceSensorVisuals[device.id]" :key="vis.nombre"
                                class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-2 min-w-0 flex-1">
                                    <span class="text-sm font-medium w-12 shrink-0"
                                        :style="{ color: SENSOR_TYPE_META[vis.tipo].color }">{{ vis.nombre }}</span>
                                    <span class="text-xs text-muted-foreground truncate">
                                        {{ latestValorFor(device.id, vis.nombre) ?? '—' }} {{
                                        SENSOR_TYPE_META[vis.tipo].unit }}
                                    </span>
                                </div>

                                <div v-if="vis.tipo === 'puerta'" class="shrink-0">
                                    <DoorClosed v-if="latestValorFor(device.id, vis.nombre) === 1" :size="24"
                                        class="text-success" />
                                    <DoorOpen v-else :size="24" class="text-warning" />
                                </div>
                                <Sparkline v-else-if="vis.values.length > 1" :data="vis.values"
                                    :color="SENSOR_TYPE_META[vis.tipo].color" :width="120" :height="28" />
                            </div>
                        </div>
                    </UiCardContent>
